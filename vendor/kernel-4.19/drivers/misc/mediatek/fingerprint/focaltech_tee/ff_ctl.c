...
extern uint16_t chip_id;
static ssize_t ff_chip_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "chip_id:0x%04x\n", chip_id);
}
static DEVICE_ATTR(chip_id, S_IRUGO | S_IWUSR, ff_chip_id_show, NULL);

static struct attribute *ff_sysfsfp_entries[] = {
    &dev_attr_chip_id.attr,
    NULL
};

static struct attribute_group ff_attribute_group = {
    .attrs = ff_sysfsfp_entries,
};
...
static int ff_ctl_report_key_event(struct input_dev *input, ff_key_event_t *kevent)
{
    ...
    input_report_key(input, KEY_PROG2, kevent->value);
    input_sync(input);
...
static int ff_ctl_register_input(void)
{
    ...
    if (g_config) {
        ...
        input_set_capability(g_context->input, EV_KEY, 558);
    }

    /* Register the allocated input device. */
    g_context->input->name = "ff_key";
	//add F11 key for focaltech-fp expand function start
    __set_bit(EV_KEY,     g_context->input->evbit );
    __set_bit(KEY_PROG2,    g_context->input->keybit);
	//add F11 key for focaltech-fp expand function end
...
static int ff_dev_cleanup(void) {
    ...
    sysfs_remove_group(&g_context->miscdev.this_device->kobj, &ff_attribute_group);	
...
static int __init ff_ctl_driver_init(void)
{
    ...
    err = sysfs_create_group(&ff_ctl_context.miscdev.this_device->kobj, &ff_attribute_group);
    if(err < 0)
    {
    	sysfs_remove_group(&ff_ctl_context.miscdev.this_device->kobj, &ff_attribute_group);
    	return err;
    }
    ...
    if (ff_ctl_init_driver() == 0) {
	    ...
	    chipid = ff_chip_init();
	    if (chipid == 0x9391 || chipid == 0x9395) {
