...
static BYTE s5kgd2sm_AWB_data[12] = {0};
...
#define OTP_DATA                s5kgd2sm_eeprom_data
#define OTP_PLATFORM_CHOICE     0x08     //bit0:awb, bit1:af, bit3:lsc  1000
...
int s5kgd2sm_get_otp_data(void)
{
	int i;
	int ret = 1;
...
//9  11  13  15 
	...
	else	//AWB Sensor
	{
		...
		for(i = 0; i < 12; i++)
		{
			s5kgd2sm_AWB_data[i] = read_cmos_sensor_8(AWB_ADDR + i);
			checksum += s5kgd2sm_AWB_data[i];
		}
	...
	//read_s5kgd2sm_PD_data();
	return ret;
}


void load_s5kgd2sm_awb(void)
{
	...
	Unit_RG   = ((s5kgd2sm_AWB_data[0] << 8) | s5kgd2sm_AWB_data[1]);
	Unit_BG   = ((s5kgd2sm_AWB_data[2] << 8) | s5kgd2sm_AWB_data[3]);
	Unit_GG   = ((s5kgd2sm_AWB_data[4] << 8) | s5kgd2sm_AWB_data[5]);
	Golden_RG = ((s5kgd2sm_AWB_data[6] << 8) | s5kgd2sm_AWB_data[7]);
	Golden_BG = ((s5kgd2sm_AWB_data[8] << 8) | s5kgd2sm_AWB_data[9]);
	Golden_GG = ((s5kgd2sm_AWB_data[10] << 8) | s5kgd2sm_AWB_data[11]);
	...
	if(Unit_RG == 0||Unit_BG==0||Golden_RG==0||Golden_BG==0)
	{
		return;
	}
