...
//BYTE s5khm2sp_DCC_data[96]= {0};
//BYTE s5khm2sp_LRC_data[384]= {0};
//YTE s5khm2sp_QSC_data[2304]= {0};
BYTE s5khm2sp_step_data[1500]={0};
static BYTE s5khm2sp_AWB_data[12] = {0};
...
#define OTP_DATA                s5khm2sp_eeprom_data
#define OTP_PLATFORM_CHOICE     0x0b     //bit0:awb, bit1:af, bit3:lsc  1010
#define OTP_FLAG_ADDR           0x0000
#define AWB_ADDR                0x000A
...
BYTE s5khm2sp_PDAF_data[1411] = {0};
...
static int read_s5khm2sp_MTK(void)
{
	int i = 0;
	kal_uint32 checksum = 0;
	int pdaf_addr = 0x771;
	int pdaf_check_addr = 0xD4D;
	int ret = 0;


	for(i = 0; i < pdaf_check_addr - pdaf_addr; i++)
	{
		s5khm2sp_step_data[i] = read_cmos_sensor_8(pdaf_addr + i);
		checksum += s5khm2sp_step_data[i];
		//LOG_INF("s5khm2sp_step_data[%d] = %u \n", i, s5khm2sp_step_data[i]);
	}
	if(CHECKSUM_METHOD(checksum, pdaf_check_addr))
	{
	...
	return ret;
	//memcpy(data, s5khm2sp_DCC_data , size);
}

int s5khm2sp_get_otp_data(void)
{
	int i;
	int ret = 1;
	kal_uint32 checksum = 0;
...
//9  11  13  15 
	...
	else	//AWB Sensor
	{
		...
		for(i = 0; i < 12; i++)
		{
			s5khm2sp_AWB_data[i] = read_cmos_sensor_8(AWB_ADDR + i);
			checksum += s5khm2sp_AWB_data[i];
		}
...
#if 0	
	if(!read_s5khm2sp_DCC())
		ret &= 0;

	if(!read_s5khm2sp_QSC())
		ret &= 0;

	if(!read_s5khm2sp_LRC())
		ret &= 0;
#endif
	if(!read_s5khm2sp_MTK())
		ret &= 0;
	//read_s5khm2sp_PD_data();
	return ret;
}


void load_s5khm2sp_awb(void)
{
	...
	Unit_RG   = ((s5khm2sp_AWB_data[0] << 8) | s5khm2sp_AWB_data[1]);
	Unit_BG   = ((s5khm2sp_AWB_data[2] << 8) | s5khm2sp_AWB_data[3]);
	Unit_GG   = ((s5khm2sp_AWB_data[4] << 8) | s5khm2sp_AWB_data[5]);
	Golden_RG = ((s5khm2sp_AWB_data[6] << 8) | s5khm2sp_AWB_data[7]);
	Golden_BG = ((s5khm2sp_AWB_data[8] << 8) | s5khm2sp_AWB_data[9]);
	Golden_GG = ((s5khm2sp_AWB_data[10] << 8) | s5khm2sp_AWB_data[11]);
	...
	write_cmos_sensor_8(0x3130, 0x01);	//s5khm2sp need
