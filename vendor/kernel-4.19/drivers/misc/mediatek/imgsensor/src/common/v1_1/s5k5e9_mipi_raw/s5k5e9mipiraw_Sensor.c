...
*************************************************************************/
#ifdef S5K5E9_OTP_ENABLE
char camera_f_info[40]; //add front camera module info
#endif
	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_INF("S5K5E9 Read sensor id fail i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//sensor_info->SensorVideoFrameRate = imgsensor_info.normal_video.max_framerate/10; /* Not used */
	//sensor_info->SensorStillCaptureFrameRate= imgsensor_info.cap.max_framerate/10; /* Not used */
	//imgsensor_info->SensorWebCamCaptureFrameRate= imgsensor_info.v.max_framerate; /* Not used */
	...
	if(local_output_format!=-1)
 		sensor_info->SensorOutputDataFormat = local_output_format;//imgsensor_info.sensor_output_dataformat;
 	else
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor_byte(0x0601, 0x01);
	} else {
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	imgsensor.shutter = shutter;
	spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
    //kal_uint16 crt_shutter = read_cmos_sensor_byte(0x0202);
    //kal_uint16 crt_framelenth = read_cmos_sensor_byte(0x0340);
    //LOG_INF("set_shutter_frame_length, read shutter[%u],read frame_length[%u]\n",crt_shutter,crt_framelenth);
    LOG_INF("set_shutter_frame_length,shutter[%u],frame_length[%u]\n",shutter,frame_length);
		...
		case SENSOR_FEATURE_GET_PIXEL_RATE:
			...
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =(imgsensor_info.cap.pclk /(imgsensor_info.cap.linelength - 80))*imgsensor_info.cap.grabwindow_width;
				break;
			case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =(imgsensor_info.normal_video.pclk /(imgsensor_info.normal_video.linelength - 80))*imgsensor_info.normal_video.grabwindow_width;
				break;
			case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =(imgsensor_info.hs_video.pclk /(imgsensor_info.hs_video.linelength - 80))*imgsensor_info.hs_video.grabwindow_width;
				break;
			case MSDK_SCENARIO_ID_SLIM_VIDEO:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =(imgsensor_info.slim_video.pclk /	(imgsensor_info.slim_video.linelength - 80))*imgsensor_info.slim_video.grabwindow_width;
				break;
			case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			default:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =(imgsensor_info.pre.pclk /(imgsensor_info.pre.linelength - 80))*imgsensor_info.pre.grabwindow_width;
				break;
			}
			break;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
 ...
 static int front_mirror_read(struct seq_file *m, void *v)
 {
 #define bufsz 512
 ...
 {
 	return single_open(file, front_mirror_read, NULL);
 };
 
 static ssize_t front_mirror_write(
 	struct file *file,
 	const char *buffer,
 	size_t count,
 	loff_t *data)
 {
 	...
 	if (sscanf(
 	    ...
 	    &mirror) == 1) {
 			printk("mirror %d\n",mirror);
 			if(mirror>=0&& mirror<4)
 				local_mirror = mirror;
 		}
 	return count;
 };
 
 static const struct file_operations fcamera_proc_fops_set_front_mirror = {
 	.owner = THIS_MODULE,
 	.open = proc_SensorType_open,
 	.read = seq_read,
 	.write = front_mirror_write,
...
UINT32 S5K5E9_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	proc_create(
 		"driver/s5k5e9_front_mirror", 0666, NULL,
 		&fcamera_proc_fops_set_front_mirror);
	return ERROR_NONE;
