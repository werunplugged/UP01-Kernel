...
extern int up_touchpanel_device_add(struct i2c_driver* mTouch, compatible_type isUsed);
extern char touch_fw_version[30];
#endif

extern char tpgesture_status;
extern bool aod_state;
/*****************************************************************************
...
int fts_wait_tp_to_valid(void)
{
    ...
    do {
        ...
        if ((ret < 0) || (idh != chip_idh) || (idl != chip_idl)) {
            FTS_DEBUG("TP Not Ready,ReadData:0x%02x%02x,0x%02x,0x%02x", idh, idl,fts_data->ic_info.ids.chip_idh,fts_data->ic_info.ids.chip_idl);
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    {
    	unsigned long lcd_w = 0, lcd_h = 0;
        kstrtoul(CONFIG_LCM_WIDTH, 10, &lcd_w);
        kstrtoul(CONFIG_LCM_HEIGHT, 10, &lcd_h);
        if(lcd_w && lcd_h){
            pdata->x_max = lcd_w;
            pdata->y_max = lcd_h;
        }
    ...
    FTS_INFO("after modify. display x(%d %d) y(%d %d)", pdata->x_min, pdata->x_max,
             pdata->y_min, pdata->y_max);
    return 0;
}
                                ...
                                unsigned long event, void *data)
{
    ...
    case FB_BLANK_UNBLANK:
        ...
        } else if (FB_EVENT_BLANK == event && !aod_state) {
...
#ifdef CONFIG_CUST_DEVICE_INFO_SUPPORT
	up_set_touch_device_used(FTS_DRIVER_NAME,DEVICE_USED);
	fts_read_reg(FTS_REG_FW_VER, &fw_version);
	//touch_fw_version = fw_version;
	sprintf(touch_fw_version,"Vno:%d",  fw_version);
#endif
...
#ifdef CONFIG_CUST_DEVICE_INFO_SUPPORT
	up_touchpanel_device_add(&fts_ts_driver,DEVICE_SUPPORTED);
#endif
    FTS_FUNC_EXIT();
    return ret;
}
