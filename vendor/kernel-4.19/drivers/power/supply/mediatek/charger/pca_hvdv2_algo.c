...
#define PCA_HVDV2_ALGO_VERSION	"1.0.0_G"
#define MS_TO_NS(msec)		((msec) * 1000 * 1000)
#define PRECISION_ENHANCE	5
#define IDVCHG_TO_HV(i)		(precise_div((i), 2))
#define VDVCHG_TO_HV(v)		((v) * 2)
#define HV_TO_IDVCHG(i)		((i) * 2)
...
#define DV2_DVCHG_VBUSALM_GAP	50	/* mV */
...
enum dv2_hvdvchg_role {
	DV2_HVDVCHG_MASTER = 0,
	DV2_HVDVCHG_SLAVE,
	DV2_HVDVCHG_MAX,
};
static const char *const dv2_dvchg_role_name[DV2_DVCHG_MAX] = {
	"master", "slave",
};

static const char *const dv2_algo_state_name[DV2_ALGO_STATE_MAX] = {
...
struct dv2_algo_desc {
	...
	u32 ita_term;			/* terminated current */
	u32 ita_step;			/* input current step */
	...
	u32 ita_ss_init;		/* SS state init input current */
	u32 ita_ss_step;		/* SS state input current step */
	u32 ita_ss_step1;		/* SS state input current step2 */
	u32 ita_ss_step2;		/* SS state input current step3 */
	u32 ita_ss_step1_vbat;		/* vbat threshold for ic_ss_step2 */
	u32 ita_ss_step2_vbat;		/* vbat threshold for ic_ss_step3 */
	...
	bool ibatocp_half;		/* make ibatocp protection half */
};
...
struct dv2_algo_data {
	...
	struct prop_chgalgo_device *pca_hv_dvchg[DV2_HVDVCHG_MAX];
	...
	enum dv2_rcable_level rcable_level;
...
static struct dv2_algo_desc algo_desc_defval = {
	...
	.ita_term = 250,
	.ita_step = 50,
	.ita_level = {1500, 1400, 1300, 1200},
	.rcable_level = {500, 536, 577, 625},
	.ita_level_dual = {2000, 1900, 1800, 1700},
	.rcable_level_dual = {375, 395, 417, 442},
	.ita_ss_init = 300,
	.ita_ss_step = 150,
	.ita_ss_step1 = 100,
	.ita_ss_step2 = 50,
	.ita_ss_step1_vbat = 4000,
	.ita_ss_step2_vbat = 4200,
	.ta_blanking = 500,
	.swchg_aicr = 0,
	.swchg_ichg = 0,
	...
	.tta_curlmt = {0, 0, 0, 0, 0, 150, 300, 450, -1},
	.tta_recovery_area = 3,
	.tbat_level_def = {0, 0, 0, 5, 25, 40, 50, 55, 60},
	.tbat_curlmt = {-1, -1, -1, 150, 0, 150, 300, 450, -1},
	.tbat_recovery_area = 3,
	.tdvchg_level_def = {0, 0, 0, 5, 25, 55, 60, 65, 70},
	.tdvchg_curlmt = {-1, -1, -1, 150, 0, 150, 300, 450, -1},
 ...
 */
static int dv2_send_notification(struct dv2_algo_info *info, unsigned long val,
...
static bool dv2_is_hwerr_notified(struct dv2_algo_info *info)
{
...
static int dv2_stop(struct dv2_algo_info *info, struct dv2_stop_info *sinfo);
static int dv2_get_adc_range(struct dv2_algo_info *info,
			     struct prop_chgalgo_device *dev,
			     ...
			     int *max)
{
	...
	if (!dev)
		return -ENODEV;
	*min = *max = 0;
	return prop_chgalgo_get_adc(dev, chan, min, max);
}

static int dv2_get_adc(struct dv2_algo_info *info,
		       struct prop_chgalgo_device *dev,
		       enum prop_chgalgo_adc_channel chan, int *val)
{
	return dv2_get_adc_range(info, dev, chan, val, val);
}

static int dv2_get_dvchg_adc(struct dv2_algo_info *info,
			     enum prop_chgalgo_adc_channel chan, int *val)
{
	struct dv2_algo_data *data = info->data;

	return dv2_get_adc(info, data->pca_dvchg[DV2_DVCHG_MASTER], chan, val);
}

static int dv2_get_dvchg_adc_slave(struct dv2_algo_info *info,
			     enum prop_chgalgo_adc_channel chan, int *val)
{
	struct dv2_algo_data *data = info->data;

	return dv2_get_adc(info, data->pca_dvchg[DV2_DVCHG_SLAVE], chan, val);
}



static int dv2_get_hv_dvchg_adc(struct dv2_algo_info *info,
				enum prop_chgalgo_adc_channel chan, int *val)
{
	struct dv2_algo_data *data = info->data;
	int ret = 0, ibus_slave = 0;

	ret = dv2_get_adc(info, data->pca_hv_dvchg[DV2_HVDVCHG_MASTER],
			chan, val);

	if ((chan == PCA_ADCCHAN_IBUS) && !ret) {
		ret = dv2_get_adc(info, data->pca_hv_dvchg[DV2_HVDVCHG_SLAVE],
			chan, &ibus_slave);
		*val += ibus_slave;
	}
	return ret;
...
static int dv2_get_hv_dvchg_adc_slave(struct dv2_algo_info *info,
				enum prop_chgalgo_adc_channel chan, int *val)
{
	struct dv2_algo_data *data = info->data;

	return dv2_get_adc(info, data->pca_hv_dvchg[DV2_HVDVCHG_SLAVE],
			chan, val);
}
 ...
 */
static inline u32 dv2_vout2vbus(struct dv2_algo_info *info, u32 vout)
{
	struct dv2_algo_data *data = info->data;
 ...
 */
static inline u32 dv2_vta_add_gap(struct dv2_algo_info *info, u32 vta)
{
 ...
 */
static inline int dv2_get_ta_cap(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
 ...
 */
static inline int dv2_get_ta_cap_by_supportive(struct dv2_algo_info *info,
					       int *vta, int *ita)
{
	int ret;
	struct dv2_algo_data *data = info->data;
    ...
    dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_IBUS, &up_ita_master);
    dv2_get_hv_dvchg_adc_slave(info, PCA_ADCCHAN_IBUS, &up_ita_slave);

    dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_IBAT, &up_ita_master);
    dv2_get_hv_dvchg_adc_slave(info, PCA_ADCCHAN_IBAT, &up_ita_slave);

    dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBAT, &up_ita_master);
    dv2_get_hv_dvchg_adc_slave(info, PCA_ADCCHAN_VBAT, &up_ita_slave);

    dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBUS, &up_ita_master);
    dv2_get_hv_dvchg_adc_slave(info, PCA_ADCCHAN_VBUS, &up_ita_slave);




    dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBUS, &up_ita_master);
    dv2_get_dvchg_adc_slave(info, PCA_ADCCHAN_IBUS, &up_ita_slave);

    dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBAT, &up_ita_master);
    dv2_get_dvchg_adc_slave(info, PCA_ADCCHAN_IBAT, &up_ita_slave);

    dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBAT, &up_ita_master);
    dv2_get_dvchg_adc_slave(info, PCA_ADCCHAN_VBAT, &up_ita_slave);

    dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBUS, &up_ita_master);
    dv2_get_dvchg_adc_slave(info, PCA_ADCCHAN_VBUS, &up_ita_slave);

	if (auth_data->support_meas_cap) {
		ret = dv2_get_ta_cap(info);
		if (ret < 0) {
	...
	ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBUS, vta);
	if (ret < 0) {
		PCA_ERR("get vbus fail(%d)\n", ret);
		return ret;
	}
	return dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_IBUS, ita);
}
...
static int dv2_get_cali_vta(struct dv2_algo_info *info, u32 ita, u32 *vta)
{
	...
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBAT, &vbat);
	if (ret < 0) {
		PCA_ERR("get vbat fail(%d)\n", ret);
		return ret;
	}
	ibat = dv2_cal_ibat(info, ita);
	vbus = dv2_vout2vbus(info, vbat + div1000(ibat * data->r_sw));
	*vta = VDVCHG_TO_HV(vbus) + (data->vbus_cali + data->vta_comp +
	...
	if (data->is_dvchg_en[DV2_DVCHG_MASTER]) {
		...
		_vta = dv2_vta_add_gap(info, data->vta_measure);
		if (_vta > *vta) {
			comp = _vta - *vta;
			data->vta_comp += comp;
 ...
 */
static int dv2_set_vbus_tracking(struct dv2_algo_info *info)
{
	int ret, vbus;
	struct dv2_algo_data *data = info->data;

	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBUS, &vbus);
	if (ret < 0) {
		PCA_ERR("get vbus fail(%d)\n", ret);
		return ret;
	}
...
static u32 dv2_get_ita_pwr_lmt_by_vta(struct dv2_algo_info *info, u32 vta)
{
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;
	u32 ita_pwr_lmt;
...
static inline u32 dv2_get_ita_tracking_max(u32 ita)
{
	return min(percent(ita, DV2_ITAOCP_RATIO),
		   (u32)(ita + DV2_ITA_TRACKING_GAP));
}
 ...
 */
static int
dv2_force_ta_cv(struct dv2_algo_info *info, struct dv2_stop_info *sinfo);
static inline int dv2_set_ta_cap_cc(struct dv2_algo_info *info, u32 vta,
				    u32 ita)
{
	int ret, vbat;
	struct dv2_algo_data *data = info->data;
	...
	while (true) {
		...
		/* Check TA's PDP */
		data->ita_pwr_lmt = dv2_get_ita_pwr_lmt_by_vta(info, vta);
		...
		PCA_DBG("vta(set,meas,comp),ita(set,meas)=(%d,%d,%d),(%d,%d)\n",
			...
			data->ita_measure);
		if (is_ta_cc) {
			opt_vta = dv2_vta_add_gap(info, data->vta_measure);
	...
	dv2_set_vbus_tracking(info);

	if (data->ita_measure > dv2_get_ita_tracking_max(data->ita_setting)) {
		ret = dv2_force_ta_cv(info, &sinfo);
		if (ret < 0)
			goto stop;
	}
	return 0;
 ...
 * calculated voltage
 */
static inline int dv2_set_ta_cap_cc_by_cali_vta(struct dv2_algo_info *info,
						u32 ita)
{
	int ret;
	u32 vta;

	ret = dv2_get_cali_vta(info, ita, &vta);
	if (ret < 0) {
		PCA_ERR("get cali vta fail(%d)\n", ret);
		return ret;
	}
	return dv2_set_ta_cap_cc(info, vta, ita);
}

static inline void dv2_update_ita_gap(struct dv2_algo_info *info, u32 ita_gap)
{
	int i;
...
static inline int dv2_set_ta_cap_cv(struct dv2_algo_info *info, u32 vta,
				    u32 ita)
{
	...
	while (true) {
		...
		ret = dv2_get_ta_cap_by_supportive(info, &vta_meas, &ita_meas_pre);
		if (ret < 0) {
		...
		ret = dv2_get_ta_cap_by_supportive(info, &vta_meas, &ita_meas_post);
		if (ret < 0) {
			PCA_ERR("get ta cap by supportive fail(%d)\n", ret);
			return ret;
		}
		...
		if (data->is_dvchg_en[DV2_DVCHG_MASTER] &&
			...
			ita_gap = precise_div(ita_meas_post - ita_meas_pre,
					      vstep_cnt);
			dv2_update_ita_gap(info, ita_gap);
			PCA_INFO("ita gap(now,updated)=(%d,%d)\n",
				 ita_gap, data->ita_gap_per_vstep);
		}
		if (ita_meas_post <= dv2_get_ita_tracking_max(ita))
			break;
		vta -= auth_data->vta_step;
...
static inline void dv2_calculate_vbat_ircmp(struct dv2_algo_info *info)
{
	int ret, ibat;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	...
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBAT, &ibat);
	if (ret < 0) {
...
static inline void dv2_select_vbat_cv(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	...
	if (data->jeita_vbat_cv > 0)
		cv_no_ircmp = min(cv_no_ircmp, (u32)data->jeita_vbat_cv);
 ...
 */
static inline int dv2_get_ita_lmt(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	...
	if (data->thermal_throttling >= 0)
		ita = min(ita, (u32)data->thermal_throttling);
...
static inline int dv2_get_idvchg_lmt(struct dv2_algo_info *info)
{
	u32 ita_lmt, idvchg_lmt;
	struct dv2_algo_data *data = info->data;

	ita_lmt = dv2_get_ita_lmt(info);
	idvchg_lmt = min(data->idvchg_cc, HV_TO_IDVCHG(ita_lmt));
	PCA_INFO("idvchg_lmt(HV_TO_IDVCHG(ita_lmt),idvchg_cc)=%d(%d,%d)\n",
		 idvchg_lmt, HV_TO_IDVCHG(ita_lmt), data->idvchg_cc);
	return idvchg_lmt;
}

/* Calculate VBUSOV S/W level */
static u32 dv2_get_dvchg_vbusovp(struct dv2_algo_info *info, u32 ita)
{
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	u32 vout, ibat;
	...
	return min(percent(dv2_vout2vbus(info, vout), DV2_VBUSOVP_RATIO),
		   data->vbusovp);
}

/* Calculate IBUSOC S/W level */
static u32 dv2_get_dvchg_ibusocp(struct dv2_algo_info *info, u32 ita)
{
	struct dv2_algo_data *data = info->data;
	u32 idvchg = HV_TO_IDVCHG(ita), ratio = DV2_IBUSOCP_RATIO;

	idvchg = (data->is_swchg_en ? (idvchg - data->aicr_setting) : idvchg);
	/* Add 10% for unbalance tolerance */
	if (data->is_dvchg_en[DV2_DVCHG_SLAVE]) {
		idvchg = precise_div(idvchg, 2);
		ratio += 10;
	}
	return percent(idvchg, ratio);
}

/* Calculate VBATOV S/W level */
static u32 dv2_get_vbatovp(struct dv2_algo_info *info)
{
	struct dv2_algo_desc *desc = info->desc;
...
static u32 dv2_get_ibatocp(struct dv2_algo_info *info, u32 ita)
{
	struct dv2_algo_data *data = info->data;
	u32 ibat;
...
static u32 dv2_get_itaocp(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
...
static int dv2_set_dvchg_protection(struct dv2_algo_info *info, bool dual)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	...
	vbusovp = percent(dv2_vout2vbus(info, vout), DV2_VBUSOVP_RATIO);
	...
	/* IBUSOCP */
	idvchg_lmt = min(data->idvchg_cc,
			 (u32)HV_TO_IDVCHG(auth_data->ita_max));
	...
	PCA_INFO("before. data->idvchg_cc = %d. desc->swchg_ichg = %d. ibatocp = %d. desc->ibatocp_half = %d. \n",
                data->idvchg_cc, desc->swchg_ichg, ibatocp, desc->ibatocp_half);
    
	if (desc->ibatocp_half)
		ibatocp = precise_div(ibatocp, 2);
	if (data->pca_dvchg[DV2_DVCHG_SLAVE] && dual) {
...
static int dv2_set_hv_dvchg_protection(struct dv2_algo_info *info, bool start)
{
	int ret = 0, i = 0;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	...
	u32 vbusovp = start ? desc->vta_cap_max : DV2_VTA_INIT;
	u32 vbatovp = start ? precise_div(desc->vta_cap_max, 2) : DV2_VTA_INIT;

	for (i; i < DV2_HVDVCHG_MAX; i++) {
		if (!data->pca_hv_dvchg[i])
			continue;
		ret = prop_chgalgo_set_ibusocp(data->pca_hv_dvchg[i],
						ibusocp + 2000);
		if (ret < 0) {
			PCA_ERR("set hv dvchg ibusocp fail(%d)\n", ret);
			return ret;
		}
		ret = prop_chgalgo_set_vbusovp(data->pca_hv_dvchg[i],
						vbusovp + 2000);
		if (ret < 0) {
			PCA_ERR("set hv dvchg vbusovp fail(%d)\n", ret);
			return ret;
		}
		ret = prop_chgalgo_set_vbatovp(data->pca_hv_dvchg[i],
						vbatovp + 2000);
		if (ret < 0) {
 ...
 * @en: enable/disable
 */
static int dv2_enable_dvchg_charging(struct dv2_algo_info *info,
...
/*
 * Set protection parameters, disable swchg and enable master divider charger
 *
 * @en: enable/disable
 */
static int dv2_set_dvchg_charging(struct dv2_algo_info *info, bool en)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	...
	if (en) {
		...
		ret = dv2_set_dvchg_protection(info, false);
		if (ret < 0) {
			PCA_ERR("set protection fail(%d)\n", ret);
			return ret;
		}
	}
	ret = dv2_enable_dvchg_charging(info, DV2_DVCHG_MASTER, en);
	if (ret < 0)
		return ret;
	if (!en) {
...
static int dv2_set_hv_dvchg_charging(struct dv2_algo_info *info, bool en)
{
	int ret = 0, i = 0;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	struct prop_chgalgo_device *dev = NULL;
	...
	for (i = 0; i < DV2_DVCHG_MAX; i++) {
		if (!data->pca_dvchg[i])
			continue;
		ret = prop_chgalgo_set_vbusovp(data->pca_dvchg[i],
				       12400); //11000
		if (ret < 0) {
			PCA_ERR("set vbusovp fail(%d), %d\n", ret, i);
			return ret;
		}
	}

	ret = prop_chgalgo_set_ta_cap(data->pca_ta, 11000, 1000);
	if (ret < 0) {
		PCA_ERR("set ta cap fail(%d)\n", ret);
		return ret;
	}
	msleep(desc->ta_blanking);
	if (en) {
		for (i = 0; i < DV2_HVDVCHG_MAX; i++) {
			dev = data->pca_hv_dvchg[i];
			if (!dev) {
				PCA_ERR("get hv_dvchg (%d) fail\n", i);
				return -ENODEV;
			}
			ret = prop_chgalgo_set_auto_trans(dev, 10000, true);
			if (ret < 0) {
				PCA_ERR("set auto trans fail(%d)\n", ret);
				return ret;
			}
		}
	} else {
		for (i = 0; i < DV2_HVDVCHG_MAX; i++) {
			...
			ret = prop_chgalgo_set_auto_trans(dev, 12000, false);    
			if (ret < 0) {
				PCA_ERR("enable auto trans fail(%d)\n", ret);
				return ret;
			}
		}
	}    
 ...
 * @en: enable/disable
 */
static int dv2_enable_swchg_charging(struct dv2_algo_info *info, bool en)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
 ...
 */
static int dv2_set_swchg_cap(struct dv2_algo_info *info, u32 aicr)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
 ...
 */
static int dv2_enable_ta_charging(struct dv2_algo_info *info, bool en)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	u32 wdt = max(desc->polling_interval * 2, (u32)DV2_TA_WDT_MIN);
	...
	ret = prop_chgalgo_enable_ta_charging(data->pca_ta, en, DV2_VTA_INIT,
					      DV2_ITA_INIT);
	if (ret < 0) {
...
static int dv2_stop(struct dv2_algo_info *info, struct dv2_stop_info *sinfo)
{
	...
	if (data->is_swchg_en)
		dv2_enable_swchg_charging(info, false);
	dv2_enable_dvchg_charging(info, DV2_DVCHG_SLAVE, false);
	dv2_set_dvchg_charging(info, false);
	if (!(data->notify & DV2_RESET_NOTIFY)) {
		dv2_set_hv_dvchg_charging(info, false);
		if (sinfo->hardreset_ta)
			...
			prop_chgalgo_set_ta_cap(data->pca_ta, DV2_VTA_INIT,
						DV2_ITA_INIT);
			dv2_enable_ta_charging(info, false);
		}
	}
	dv2_set_hv_dvchg_protection(info, false);
	dv2_send_notification(info, PCA_NOTIEVT_ALGO_STOP, &notify);
	PCA_DBG("--\n");
	return 0;
}

static inline int dv2_init_algo_data(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;
	...
	data->rcable_level = DV2_RCABLE_NORMAL;
	data->ita_lmt = min(ita_level[DV2_RCABLE_NORMAL],
			    (u32)auth_data->ita_max);
	data->ita_ss_init = max(data->ita_ss_init, (u32)auth_data->ita_min);
	data->ita_ss_init= min(data->ita_ss_init, data->ita_lmt);
	data->ita_pwr_lmt = 0;
	data->idvchg_cc = HV_TO_IDVCHG(ita_level[DV2_RCABLE_NORMAL]) -
			  desc->swchg_aicr;
	data->ita_term = desc->ita_term;
...
static int dv2_earily_restart(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;

	ret = dv2_enable_dvchg_charging(info, DV2_DVCHG_SLAVE, false);
	if (ret < 0) {
		PCA_ERR("disable slave dvchg fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_enable_dvchg_charging(info, DV2_DVCHG_MASTER, false);
	if (ret < 0) {
		PCA_ERR("disable master dvchg fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_set_hv_dvchg_charging(info, false);
	if (ret < 0) {
		PCA_ERR("disable hv dvchg charging fail(%d)\n", ret);
		return ret;
	}
	if (auth_data->support_cc) {
		ret = dv2_set_ta_cap_cc(info, DV2_VTA_INIT, DV2_ITA_INIT);
		if (ret < 0) {
			PCA_ERR("set ta cap fail(%d)\n", ret);
			return ret;
		}
	}
	ret = dv2_enable_ta_charging(info, false);
	if (ret < 0) {
		PCA_ERR("disable ta charging fail(%d)\n", ret);
		return ret;
	}
	return dv2_init_algo_data(info);
}
 ...
 */
static inline int dv2_start(struct dv2_algo_info *info)
{
	int ret, ibus, vbat, vbus, ita, i, j = 0;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;
	...
	for (j; j < DV2_HVDVCHG_MAX; j++) {
		if (data->pca_hv_dvchg[j]) {
			ret = prop_chgalgo_init_chip(data->pca_hv_dvchg[j]);
			if (ret < 0) {
				PCA_ERR("init hv dvchg chip fail(%d)\n", ret);
				return ret;
			}
			ret = dv2_set_hv_dvchg_protection(info, true);
			if (ret < 0) {
	...
	data->ita_ss_init = desc->ita_ss_init;
	...
	msleep(1000);
	ret = dv2_get_adc(info, data->pca_swchg, PCA_ADCCHAN_VBUS, &vbus);
	if (ret < 0) {
		PCA_ERR("get swchg vbus fail(%d)\n", ret);
		goto start;
	}
	ret = dv2_get_adc(info, data->pca_swchg, PCA_ADCCHAN_IBUS, &ibus);
	if (ret < 0) {
		PCA_ERR("get swchg ibus fail(%d)\n", ret);
		goto start;
	}
	ret = dv2_get_adc(info, data->pca_swchg, PCA_ADCCHAN_VBAT, &vbat);
	if (ret < 0) {
		PCA_ERR("get swchg vbat fail(%d)\n", ret);
		goto start;
	}
	ita = precise_div(percent(vbus * ibus, 90), 4 * vbat);
	if (ita < desc->ita_term) {
		PCA_ERR("estimated ita(%d) < ita_term(%d)\n", ita,
			desc->ita_term);
		return -EINVAL;
	}
	/* Update ita_ss_init */
	if (ita >= auth_data->ita_min) {
		PCA_INFO("set ita_ss_init(%d)->(%d)\n", desc->ita_ss_init, ita);
		data->ita_ss_init = ita;
	}
start:
	/* disable charger */
	ret = prop_chgalgo_enable_charging(data->pca_swchg, false);
...
static int dv2_algo_init_with_ta_cc(struct dv2_algo_info *info)
{
	int ret, i, vbus, vbat;
	...
	ret = dv2_enable_ta_charging(info, true);
	if (ret < 0) {
	...
	ret = dv2_set_hv_dvchg_charging(info, true);
	if (ret < 0) {
		PCA_ERR("set hv dvchg charging fail(%d)\n", ret);
		goto err;
	}

	/* Initial setting, no need to check ita_lmt */
	ret = dv2_set_ta_cap_cc_by_cali_vta(info, data->ita_ss_init);
	if (ret < 0) {
	...
	for (i = 0; i < avg_times; i++) {
		...
		ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBUS, &vbus);
		if (ret < 0) {
			PCA_ERR("get vbus fail(%d)\n", ret);
			goto err;
		}
	...
	ret = dv2_set_dvchg_charging(info, true);
	if (ret < 0) {
...
out:
	return dv2_stop(info, &sinfo);
}

static int dv2_algo_init_with_ta_cv(struct dv2_algo_info *info)
{
	...
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VOUT, &vout);
	if (ret < 0) {
	...
	vta = VDVCHG_TO_HV(dv2_vout2vbus(info, vout));
	ret = dv2_set_ta_cap_cv(info, vta, data->ita_ss_init);
	if (ret < 0) {
		PCA_ERR("set ta cap fail(%d)\n", ret);
		goto err;
	}
	...
	for (i = 0; i < avg_times; i++) {
		if (auth_data->support_meas_cap) {
			...
			ita_avg += data->ita_measure;
			vta_avg += data->vta_measure;
			ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBUS,
						   &vbus);
			if (ret < 0) {
				PCA_ERR("get vbus fail(%d)\n", ret);
				goto err;
	...
	ret = dv2_get_ta_cap_by_supportive(info, &data->vta_measure,
					   &data->ita_measure);
	if (ret < 0) {
		PCA_ERR("get ta cap fail(%d)\n", ret);
 ...
 */
static int dv2_algo_init(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;

	return auth_data->support_cc ? dv2_algo_init_with_ta_cc(info) :
				       dv2_algo_init_with_ta_cv(info);
}
...
static int dv2_algo_get_r_info(struct dv2_algo_info *info,
			       ...
			       struct dv2_stop_info *sinfo)
{
	...
	ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBUS, &r_info->hv_vbus);
	if (ret < 0) {
		PCA_ERR("get hv vbus fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_IBUS, &r_info->hv_ibus);
	if (ret < 0) {
		PCA_ERR("get hv ibus fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBUS, &r_info->vbus);
	if (ret < 0) {
		PCA_ERR("get vbus fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBUS, &r_info->ibus);
	if (ret < 0) {
		PCA_ERR("get ibus fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VOUT, &r_info->vout);
	if (ret < 0) {
		PCA_ERR("get vout fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBAT, &r_info->vbat);
	if (ret < 0) {
		PCA_ERR("get vbat fail(%d)\n", ret);
		return ret;
	}
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBAT, &r_info->ibat);
	if (ret < 0) {
		PCA_ERR("get ibat fail(%d)\n", ret);
		return ret;
	}
	PCA_DBG("vta:%d,ita:%d,vbus:%d,%d,ibus:%d,%d,vout:%d,vbat:%d,ibat:%d\n",
		r_info->vta, r_info->ita, r_info->hv_vbus, r_info->vbus,
		r_info->hv_ibus, r_info->ibus, r_info->vout, r_info->vbat,
		r_info->ibat);
	return 0;
}

static int dv2_algo_cal_r_info_with_ta_cap(struct dv2_algo_info *info,
					   struct dv2_stop_info *sinfo)
{
	int ret, i;
	struct dv2_algo_data *data = info->data;
	...
	for (i = 0; i < DV2_ALGO_MEASURE_R_AVG_TIMES + 2; i++) {
		...
		ret = dv2_algo_get_r_info(info, &r_info, sinfo);
		if (ret < 0) {
	...
	return 0;
stop:
	dv2_stop(info, &_sinfo);
	return -EIO;
}

static int dv2_select_ita_lmt_by_r(struct dv2_algo_info *info, bool dual)
{
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;
	...
	if (data->r_cable <= rcable_level[DV2_RCABLE_NORMAL])
		...
		data->rcable_level = DV2_RCABLE_BAD1;
	else if (data->r_cable <= rcable_level[DV2_RCABLE_BAD2])
		data->rcable_level = DV2_RCABLE_BAD2;
	else if (data->r_cable <= rcable_level[DV2_RCABLE_BAD3])
		data->rcable_level = DV2_RCABLE_BAD3;
	else {
		PCA_ERR("r_cable(%d) too worse\n", data->r_cable);
		return -EINVAL;
	}
out:
	ita_lmt_by_r = ita_level[data->rcable_level];
	PCA_INFO("ita limited by r = %d\n", ita_lmt_by_r);
	data->ita_lmt = min(ita_lmt_by_r, (u32)auth_data->ita_max);
	data->ita_pwr_lmt = dv2_get_ita_pwr_lmt_by_vta(info, data->vta_setting);
	ita_lmt = dv2_get_ita_lmt(info);
	if (ita_lmt < data->ita_term) {
		PCA_ERR("ita_lmt(%d) < ita_term(%d)\n", ita_lmt,
			data->ita_term);
		return -EINVAL;
	}
	return 0;
...
static int dv2_algo_measure_r_with_ta_cc(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
	...
	ret = dv2_algo_cal_r_info_with_ta_cap(info, &sinfo);
	if (ret < 0)
		goto err;
	...
	if (data->pca_dvchg[DV2_DVCHG_SLAVE] && !data->tried_dual_dvchg &&
		...
		data->ita_term = 2 * desc->ita_term;
		data->idvchg_cc =
			HV_TO_IDVCHG(desc->ita_level_dual[data->rcable_level]) -
			desc->swchg_aicr;
		ret = dv2_select_ita_lmt_by_r(info, true);
		if (ret < 0) {
		...
		/*
		 * Turn on slave dvchg
		 * only if idvchg_lmt >= HV_TO_IDVCHG(ita_term)
		 */
		idvchg_lmt = dv2_get_idvchg_lmt(info);
		if (idvchg_lmt < HV_TO_IDVCHG(data->ita_term)) {
			PCA_ERR("idvchg_lmt(%d) < HV_TO_IDVCHG(ita_term)(%d)\n",
				idvchg_lmt, HV_TO_IDVCHG(data->ita_term));
			goto single_dvchg_select_ita;
		}
		ret = dv2_enable_dvchg_charging(info, DV2_DVCHG_MASTER, false);
		if (ret < 0) {
		...
		ret = dv2_set_dvchg_protection(info, true);
		if (ret < 0) {
			PCA_ERR("set dual dvchg protection fail(%d)\n", ret);
			goto single_dvchg_restart;
		}
		ret = dv2_enable_dvchg_charging(info, DV2_DVCHG_SLAVE, true);
		if (ret < 0) {
			PCA_ERR("en slave dvchg fail(%d)\n", ret);
			goto single_dvchg_restart;
		}
		ita = max(data->ita_term, data->ita_setting);
		ita = min(ita, IDVCHG_TO_HV(idvchg_lmt));
		ret = dv2_set_ta_cap_cc_by_cali_vta(info, ita);
		if (ret < 0) {
			PCA_ERR("set ta cap fail(%d)\n", ret);
			sinfo.hardreset_ta = true;
			goto out;
		}
		ret = dv2_enable_dvchg_charging(info, DV2_DVCHG_MASTER, true);
		if (ret < 0) {
...
single_dvchg_select_ita:
	data->ita_term = desc->ita_term;
	data->idvchg_cc = HV_TO_IDVCHG(desc->ita_level[data->rcable_level]) -
			  desc->swchg_aicr;
	ret = dv2_select_ita_lmt_by_r(info, false);
	if (ret < 0) {
...
static int dv2_algo_measure_r_with_ta_cv(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
...
static int dv2_algo_measure_r(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;

	return auth_data->support_cc ? dv2_algo_measure_r_with_ta_cc(info) :
				       dv2_algo_measure_r_with_ta_cv(info);
}

static int dv2_set_slave_dvchg_off(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
...
static int dv2_force_ta_cv(struct dv2_algo_info *info,
			   struct dv2_stop_info *sinfo)
{
	int ret;
	u32 ita, vta;
	...
	ret = dv2_set_ta_cap_cv(info, vta, ita);
	if (ret < 0) {
...
static int dv2_check_force_ta_cv(struct dv2_algo_info *info,
				 struct dv2_stop_info *sinfo)
{
	int ret;
	u32 vbat;
...
static int dv2_algo_ss_dvchg_with_ta_cc(struct dv2_algo_info *info)
{
	int ret, vbat;
	...
	ret = dv2_check_force_ta_cv(info, &sinfo);
	if (ret < 0) {
	...
	/*
	 * If idvchg cannot drain more than HV_TO_IDVCHG(data->ita_term),
	 * stop dv2
	 */
	idvchg_lmt = dv2_get_idvchg_lmt(info);
	if (idvchg_lmt < HV_TO_IDVCHG(data->ita_term)) {
		PCA_INFO("idvchg_lmt(%d) < HV_TO_IDVCHG(ita_term)(%d)\n",
			 idvchg_lmt, HV_TO_IDVCHG(data->ita_term));
		goto err;
	}
	...
	if (vbat >= data->vbat_cv) {
		if (data->ita_measure < data->ita_term) {
			if (data->is_dvchg_en[DV2_DVCHG_SLAVE]) {
				ret = dv2_set_slave_dvchg_off(info);
				if (ret < 0) {
...
cc_cv:
		ita = min(data->ita_setting - desc->ita_ss_step,
			  IDVCHG_TO_HV(idvchg_lmt));
		data->state = DV2_ALGO_CC_CV;
		goto out_set_cap;
	}

	/* ITA reaches CC level */
	if (HV_TO_IDVCHG(data->ita_measure) >= idvchg_lmt ||
	    HV_TO_IDVCHG(data->ita_setting) >= idvchg_lmt) {
		/*
		...
		if (vbat < desc->swchg_off_vbat && desc->swchg_aicr > 0 &&
		    desc->swchg_ichg > 0 &&
		    (HV_TO_IDVCHG(ita_lmt) >
		     data->idvchg_cc + desc->swchg_aicr_ss_init)) {
			ret = dv2_set_swchg_cap(info,
						desc->swchg_aicr_ss_init);
			if (ret < 0) {
				PCA_ERR("set swchg cap fail(%d)\n", ret);
				goto err;
			}
			ret = dv2_enable_swchg_charging(info, true);
			if (ret < 0) {
				PCA_ERR("en swchg fail(%d)\n", ret);
				goto err;
			}
			ita = IDVCHG_TO_HV(idvchg_lmt +
					   desc->swchg_aicr_ss_init);
			data->aicr_init_lmt = HV_TO_IDVCHG(ita_lmt) -
					      data->idvchg_cc;
	...
	if (vbat < desc->ita_ss_step1_vbat)
		ita = data->ita_setting + desc->ita_ss_step;
	else if (vbat < desc->ita_ss_step2_vbat)
		ita = data->ita_setting + desc->ita_ss_step1;
	else
		ita = data->ita_setting + desc->ita_ss_step2;
	ita = min(ita, IDVCHG_TO_HV(idvchg_lmt));
...
static int dv2_algo_ss_dvchg_with_ta_cv(struct dv2_algo_info *info)
{
	int ret, vbat;
...
repeat:
	...
	/* Turn on slave dvchg if idvchg_lmt >= HV_TO_IDVCHG(ita_term) */
	ita = data->ita_term * 2;
	if (data->pca_dvchg[DV2_DVCHG_SLAVE] && !data->tried_dual_dvchg &&
...
single_dvchg_select_ita:
		data->ita_term = desc->ita_term;
		data->idvchg_cc =
			HV_TO_IDVCHG(desc->ita_level[data->rcable_level]) -
			desc->swchg_aicr;
		ret = dv2_select_ita_lmt_by_r(info, false);
		if (ret < 0) {
			PCA_ERR("select dvchg ita lmt fail(%d)\n", ret);
...
cc_cv:
	...
	if (HV_TO_IDVCHG(data->ita_measure + ita_gap_per_vstep) > idvchg_lmt ||
	    vta == auth_data->vcap_max)
		data->state = DV2_ALGO_CC_CV;
	else {
		vta += auth_data->vta_step;
...
static int dv2_algo_ss_dvchg(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;

	return (auth_data->support_cc && !data->force_ta_cv) ?
		dv2_algo_ss_dvchg_with_ta_cc(info) :
		dv2_algo_ss_dvchg_with_ta_cv(info);
}

static int dv2_check_swchg_off(struct dv2_algo_info *info,
			       struct dv2_stop_info *sinfo)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	...
	if (aicr >= desc->swchg_aicr_ss_init) {
		ita = data->ita_setting -
		      IDVCHG_TO_HV(data->aicr_setting - aicr);
	} else
		ita = data->ita_setting - IDVCHG_TO_HV(data->aicr_setting);
	ret = dv2_set_ta_cap_cc_by_cali_vta(info, ita);
	if (ret < 0) {
		PCA_ERR("set ta cap fail(%d)\n", ret);
		sinfo->hardreset_ta = true;
	...
	return (aicr >= desc->swchg_aicr_ss_init) ?
		dv2_set_swchg_cap(info, aicr) :
		dv2_enable_swchg_charging(info, false);
}

static int dv2_update_aicr_lmt(struct dv2_algo_info *info)
{
	struct dv2_algo_desc *desc = info->desc;
	struct dv2_algo_data *data = info->data;
	u32 ita_lmt;
	...
	if (HV_TO_IDVCHG(ita_lmt) <
	    data->idvchg_cc + desc->swchg_aicr_ss_init) {
		data->aicr_lmt = 0;
		return -EINVAL;
	}
	data->aicr_lmt = min(data->aicr_lmt,
			     HV_TO_IDVCHG(ita_lmt) - data->idvchg_cc);
...
static int dv2_algo_ss_swchg(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_desc *desc = info->desc;
	struct dv2_algo_data *data = info->data;
	...
	if (dv2_update_aicr_lmt(info) < 0)
		goto out;
	/* Set new AICR & TA cap */
	aicr = min(data->aicr_setting + desc->swchg_aicr_ss_step,
		   data->aicr_lmt);
	ita += IDVCHG_TO_HV(aicr - data->aicr_setting);
	ret = dv2_set_swchg_cap(info, aicr);
	if (ret < 0) {
		PCA_ERR("set swchg cap fail(%d)\n", ret);
		goto err;
	}
...
out:
	...
	ret = dv2_check_swchg_off(info, &sinfo);
	if (ret < 0) {
...
static int dv2_algo_cc_cv_with_ta_cc(struct dv2_algo_info *info)
{
	int ret, vbat = 0;
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
...
cc_cv:
	...
	ita_lmt = min(ita_lmt, IDVCHG_TO_HV(data->is_swchg_en ?
		      (data->idvchg_cc + data->aicr_setting) :
		      data->idvchg_cc));
	if (ita_lmt < data->ita_term) {
		PCA_INFO("ita_lmt(%d) < ita_term(%d)\n", ita_lmt,
			 data->ita_term);
		goto err;
	}

	if (vbat >= data->vbat_cv) {
		ita = data->ita_setting - desc->ita_step;
		data->is_vbat_over_cv = true;
	} else if (vbat < desc->ita_ss_step1_vbat && ita < ita_lmt) {
		PCA_INFO("++ita(set,lmt,add)=(%d,%d,%d)\n", ita, ita_lmt,
			 desc->ita_ss_step);
		ita = data->ita_setting + desc->ita_ss_step;
	} else if (vbat < desc->ita_ss_step2_vbat && ita < ita_lmt) {
		PCA_INFO("++ita(set,lmt,add)=(%d,%d,%d)\n", ita, ita_lmt,
			 desc->ita_ss_step1);
		ita = data->ita_setting + desc->ita_ss_step1;
...
static int dv2_algo_cc_cv_with_ta_cv(struct dv2_algo_info *info)
{
	int ret, vbat;
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;
...
cc_cv:
	...
	if (vbat >= data->vbat_cv) {
		...
		ita -= ita_gap_per_vstep;
		data->is_vbat_over_cv = true;
	} else if (HV_TO_IDVCHG(data->ita_measure) > idvchg_lmt) {
		vta -= auth_data->vta_step;
		ita -= ita_gap_per_vstep;
		ita = min(ita, IDVCHG_TO_HV(idvchg_lmt));
		PCA_INFO("--vta, ita(meas,lmt)=(%d,%d)\n", data->ita_measure,
		   ...
		   (data->ita_measure <=
		   IDVCHG_TO_HV(idvchg_lmt) - ita_gap_per_vstep) &&
	...
	ret = dv2_get_ta_cap_by_supportive(info, &vta_measure, &ita_measure);
	if (ret < 0) {
		PCA_ERR("get ta cap fail(%d)\n", ret);
		sinfo.hardreset_ta = auth_data->support_meas_cap;
		goto out;
...
static int dv2_algo_cc_cv(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct prop_chgalgo_ta_auth_data *auth_data = &data->ta_auth_data;

	return (auth_data->support_cc && !data->force_ta_cv) ?
		dv2_algo_cc_cv_with_ta_cc(info) :
		dv2_algo_cc_cv_with_ta_cv(info);
}
 ...
 */
static bool dv2_check_ta_status(struct dv2_algo_info *info,
				struct dv2_stop_info *sinfo)
{
	int ret;
...
static bool dv2_check_dvchg_ibusocp(struct dv2_algo_info *info,
				    struct dv2_stop_info *sinfo)
{
	...
	if (!data->is_dvchg_en[DV2_DVCHG_MASTER])
		return true;
	ibusocp = dv2_get_dvchg_ibusocp(info, data->ita_setting);
	for (i = DV2_DVCHG_MASTER; i < DV2_DVCHG_MAX; i++) {
		if (!data->is_dvchg_en[i])
			continue;
		prop_chgalgo_get_adc_accuracy(data->pca_dvchg[i],
					      PCA_ADCCHAN_IBUS, &acc, &acc);
		ret = dv2_get_adc(info, data->pca_dvchg[i], PCA_ADCCHAN_IBUS,
				  &ibus);
		if (ret < 0) {
			PCA_ERR("get ibus fail(%d)\n", ret);
			return false;
		...
		PCA_INFO("(%s)ibus(%d+-%dmA), ibusocp(%dmA)\n",
			 dv2_dvchg_role_name[i], ibus, acc, ibusocp);
		if (ibus > acc)
			ibus -= acc;
		if (ibus > ibusocp) {
			PCA_ERR("(%s)ibus(%dmA) > ibusocp(%dmA)\n",
				dv2_dvchg_role_name[i], ibus, ibusocp);
			return false;
		}
	}
	return true;
...
static bool dv2_check_ta_ibusocp(struct dv2_algo_info *info,
				 struct dv2_stop_info *sinfo)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	...
	if (data->ita_measure > itaocp) {
		...
		if (!dv2_check_dvchg_ibusocp(info, sinfo))
			goto err;
	}
	return true;
err:
 ...
 */
static bool dv2_check_dvchg_vbusovp(struct dv2_algo_info *info,
				    struct dv2_stop_info *sinfo)
{
	int ret, vbus, i;
	struct dv2_algo_data *data = info->data;
	...
	vbusovp = dv2_get_dvchg_vbusovp(info, data->ita_setting);
	for (i = DV2_DVCHG_MASTER; i < DV2_DVCHG_MAX; i++) {
		if (!data->is_dvchg_en[i])
			continue;
		ret = dv2_get_adc(info, data->pca_dvchg[i], PCA_ADCCHAN_VBUS,
				  &vbus);
		if (ret < 0) {
			PCA_ERR("get vbus fail(%d)\n", ret);
			return false;
		...
		PCA_INFO("(%s)vbus(%dmV), vbusovp(%dmV)\n",
			 dv2_dvchg_role_name[i], vbus, vbusovp);
		if (vbus > vbusovp) {
...
static bool dv2_check_vbatovp(struct dv2_algo_info *info,
			      struct dv2_stop_info *sinfo)
{
	int ret, vbat;
	u32 vbatovp;

	vbatovp = dv2_get_vbatovp(info);
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBAT, &vbat);
	if (ret < 0) {
		PCA_ERR("get vbat fail(%d)\n", ret);
		return false;
...
static bool dv2_check_ibatocp(struct dv2_algo_info *info,
			      struct dv2_stop_info *sinfo)
{
	int ret, ibat;
	struct dv2_algo_data *data = info->data;
	...
	if (!data->is_dvchg_en[DV2_DVCHG_MASTER])
		return true;
	ibatocp =  dv2_get_ibatocp(info, data->ita_setting);
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBAT, &ibat);
	if (ret < 0) {
		PCA_ERR("get ibat fail(%d)\n", ret);
		return false;
...
static bool dv2_check_thermal_level(struct dv2_algo_info *info,
 ...
 */
static bool dv2_check_tbat_level(struct dv2_algo_info *info,
				 struct dv2_stop_info *sinfo)
{
	int ret, tbat;
	struct dv2_algo_data *data = info->data;
	...
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_TBAT, &tbat);
	if (ret < 0) {
		PCA_ERR("get tbat fail(%d)\n", ret);
		return false;
	}
	tdata.temp = tbat;
	return dv2_check_thermal_level(info, &tdata);
}
 ...
 * otherwise return true
 */
static bool dv2_check_tta_level(struct dv2_algo_info *info,
				struct dv2_stop_info *sinfo)
{
	int ret, tta;
	struct dv2_algo_data *data = info->data;
 ...
 * otherwise return true
 */
static bool dv2_check_tdvchg_level(struct dv2_algo_info *info,
				   struct dv2_stop_info *sinfo)
{
	int ret, i, tdvchg;
	struct dv2_algo_data *data = info->data;
	...
	for (i = DV2_DVCHG_MASTER; i < DV2_DVCHG_MAX; i++) {
		if (!data->is_dvchg_en[i])
			continue;
		ret = dv2_get_adc(info, data->pca_dvchg[i], PCA_ADCCHAN_TCHG,
				  &tdvchg);
		if (ret < 0) {
			PCA_ERR("get tdvchg fail(%d)\n", ret);
			return false;
		}
		snprintf(buf, 8 + strlen(dv2_dvchg_role_name[i]), "tdvchg_%s",
			 dv2_dvchg_role_name[i]);
		tdata.name = buf;
		tdata.temp = tdvchg;
		if (!dv2_check_thermal_level(info, &tdata))
			return false;
	}
	return true;
}
 ...
 * otherwise return true
 */
static bool dv2_check_tswchg_level(struct dv2_algo_info *info,
				   struct dv2_stop_info *sinfo)
{
	int ret, tswchg;
	struct dv2_algo_data *data = info->data;
	...
	ret = dv2_get_adc(info, data->pca_swchg, PCA_ADCCHAN_TCHG, &tswchg);
	if (ret < 0) {
...
static bool
(*dv2_safety_check_fn[])(struct dv2_algo_info *info,
			 struct dv2_stop_info *sinfo) = {
	dv2_check_ta_status,
	dv2_check_ta_ibusocp,
	dv2_check_dvchg_vbusovp,
	dv2_check_dvchg_ibusocp,
	dv2_check_vbatovp,
	dv2_check_ibatocp,
	dv2_check_tbat_level,
	dv2_check_tta_level,
	dv2_check_tdvchg_level,
	dv2_check_tswchg_level,
};

static bool dv2_algo_safety_check(struct dv2_algo_info *info)
{
	int i;
	struct dv2_stop_info sinfo = {
		.reset_ta = true,
	...
	PCA_DBG("++\n");
	for (i = 0; i < ARRAY_SIZE(dv2_safety_check_fn); i++) {
		if (!dv2_safety_check_fn[i](info, &sinfo))
			goto err;
	}
	return true;
...
static bool dv2_is_ta_rdy(struct dv2_algo_info *info)
{
	int ret, i;
	struct dv2_algo_desc *desc = info->desc;
	struct dv2_algo_data *data = info->data;
...
static inline void dv2_wakeup_algo_thread(struct dv2_algo_data *data)
{
	PCA_DBG("++\n");
...
dv2_algo_timer_cb(struct alarm *alarm, ktime_t now)
{
	struct dv2_algo_data *data =
		container_of(alarm, struct dv2_algo_data, timer);

	PCA_DBG("++\n");
	dv2_wakeup_algo_thread(data);
 ...
 */
static bool dv2_algo_check_charging_time(struct dv2_algo_info *info)
{
	struct dv2_algo_data *data = info->data;
	struct dv2_algo_desc *desc = info->desc;
...
static int dv2_notify_hardreset_hdlr(struct dv2_algo_info *info)
{
	struct dv2_stop_info sinfo = {
		.reset_ta = false,
		.hardreset_ta = false,
...
static int dv2_notify_detach_hdlr(struct dv2_algo_info *info)
{
	struct dv2_stop_info sinfo = {
		.reset_ta = false,
		.hardreset_ta = false,
...
static int dv2_notify_hwerr_hdlr(struct dv2_algo_info *info)
{
	struct dv2_stop_info sinfo = {
		.reset_ta = true,
		.hardreset_ta = false,
...
static int dv2_notify_ibusucpf_hdlr(struct dv2_algo_info *info)
{
	int ret, ibus;
	struct dv2_algo_data *data = info->data;
	...
	/* Last chance */
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_IBUS, &ibus);
	if (ret < 0) {
	...
	return 0;
out:
	return dv2_notify_hwerr_hdlr(info);
}

static int dv2_notify_vbatovp_alarm_hdlr(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_stop_info sinfo = {
...
static int dv2_notify_vbusovp_alarm_hdlr(struct dv2_algo_info *info)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	struct dv2_stop_info sinfo = {
...
static int
(*dv2_notify_pre_hdlr[PCA_NOTIEVT_MAX])(struct dv2_algo_info *info) = {
	[PCA_NOTIEVT_DETACH] = dv2_notify_detach_hdlr,
	[PCA_NOTIEVT_HARDRESET] = dv2_notify_hardreset_hdlr,
	[PCA_NOTIEVT_VBUSOVP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_IBUSOCP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_IBUSUCP_FALL] = dv2_notify_ibusucpf_hdlr,
	[PCA_NOTIEVT_VBATOVP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_IBATOCP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_VOUTOVP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_VDROVP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_VBATOVP_ALARM] = dv2_notify_vbatovp_alarm_hdlr,
};

static int
(*dv2_notify_post_hdlr[PCA_NOTIEVT_MAX])(struct dv2_algo_info *info) = {
	[PCA_NOTIEVT_DETACH] = dv2_notify_detach_hdlr,
	[PCA_NOTIEVT_HARDRESET] = dv2_notify_hardreset_hdlr,
	[PCA_NOTIEVT_VBUSOVP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_IBUSOCP] = dv2_notify_hwerr_hdlr,
	...
	[PCA_NOTIEVT_VDROVP] = dv2_notify_hwerr_hdlr,
	[PCA_NOTIEVT_VBATOVP_ALARM] = dv2_notify_vbatovp_alarm_hdlr,
	[PCA_NOTIEVT_VBUSOVP_ALARM] = dv2_notify_vbusovp_alarm_hdlr,
};

static int dv2_pre_handle_notify_evt(struct dv2_algo_info *info)
{
	int i;
	struct dv2_algo_data *data = info->data;
	...
	for (i = 0; i < PCA_NOTIEVT_MAX; i++) {
		if ((data->notify & BIT(i)) && dv2_notify_pre_hdlr[i]) {
			data->notify &= ~BIT(i);
			mutex_unlock(&data->notify_lock);
			dv2_notify_pre_hdlr[i](info);
			mutex_lock(&data->notify_lock);
		}
	}
	mutex_unlock(&data->notify_lock);
...
static int dv2_post_handle_notify_evt(struct dv2_algo_info *info)
{
	int i;
	struct dv2_algo_data *data = info->data;
	...
	PCA_DBG("0x%08X\n", data->notify);
	for (i = 0; i < PCA_NOTIEVT_MAX; i++) {
		if ((data->notify & BIT(i)) && dv2_notify_post_hdlr[i]) {
			data->notify &= ~BIT(i);
			mutex_unlock(&data->notify_lock);
			dv2_notify_post_hdlr[i](info);
			mutex_lock(&data->notify_lock);
		}
	}
	mutex_unlock(&data->notify_lock);
...
static int dv2_dump_charging_info(struct dv2_algo_info *info)
{
	int ret, i;
	int vbus, hv_vbus, ibus[DV2_DVCHG_MAX] = {0}, hv_ibus, ibus_swchg = 0;
	int vbat, ibat;
	struct dv2_algo_data *data = info->data;

	ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_VBUS, &hv_vbus);
	if (ret < 0)
		PCA_ERR("get hv vbus fail\n");
	ret = dv2_get_hv_dvchg_adc(info, PCA_ADCCHAN_IBUS, &hv_ibus);
	if (ret < 0)
		PCA_ERR("get hv ibus fail\n");
	ret = dv2_get_dvchg_adc(info, PCA_ADCCHAN_VBUS, &vbus);
	if (ret < 0)
		PCA_ERR("get vbus fail\n");
	for (i = 0; i < DV2_DVCHG_MAX; i++) {
	...
	if (data->is_swchg_en) {
		ret = dv2_get_adc(info, data->pca_swchg, PCA_ADCCHAN_IBUS,
				  &ibus_swchg);
		if (ret < 0)
...
static int dv2_algo_threadfn(void *param)
{
	...
	while (!kthread_should_stop()) {
		...
		PCA_INFO("state = %s\n", dv2_algo_state_name[data->state]);
		if (atomic_read(&data->stop_algo))
			dv2_stop(info, &sinfo);
		dv2_pre_handle_notify_evt(info);
		if (data->state != DV2_ALGO_STOP) {
			dv2_algo_check_charging_time(info);
			dv2_calculate_vbat_ircmp(info);
			dv2_select_vbat_cv(info);
			dv2_dump_charging_info(info);
		}
		switch (data->state) {
		case DV2_ALGO_INIT:
			dv2_algo_init(info);
		...
		case DV2_ALGO_MEASURE_R:
			dv2_algo_measure_r(info);
			break;
		case DV2_ALGO_SS_SWCHG:
			dv2_algo_ss_swchg(info);
			break;
		case DV2_ALGO_SS_DVCHG:
			dv2_algo_ss_dvchg(info);
			break;
		case DV2_ALGO_CC_CV:
			dv2_algo_cc_cv(info);
			break;
		...
		dv2_post_handle_notify_evt(info);
		if (data->state != DV2_ALGO_STOP) {
			if (!dv2_algo_safety_check(info))
				goto cont;
			dv2_dump_charging_info(info);
			if (data->state == DV2_ALGO_CC_CV &&
...
cont:
		PCA_INFO("end before get lock\n");
		mutex_unlock(&data->lock);
		PCA_INFO("end after get lock\n");
		pm_relax(info->dev);
	}
	return 0;
}
...
{ 
	...
	data->pca_hv_dvchg[DV2_HVDVCHG_MASTER] =
		prop_chgalgo_dev_get_by_name("pca_chg_hv_dvchg");
	if (!data->pca_hv_dvchg[DV2_HVDVCHG_MASTER]) {
		PCA_ERR("get pca_hv_dvchg fail\n");
		ret = -ENODEV;
		goto out;
	}
	data->pca_hv_dvchg[DV2_HVDVCHG_SLAVE] =
		prop_chgalgo_dev_get_by_name("pca_chg_hv_dvchg_slave");
	if (!data->pca_hv_dvchg[DV2_HVDVCHG_SLAVE]) {
		PCA_ERR("get pca_hv_dvchg_slave fail\n");
		ret = -ENODEV;
		goto out;
	}
	data->pca_dvchg[DV2_DVCHG_MASTER] =
...
static bool dv2_is_algo_init(struct prop_chgalgo_device *pca)
{
	struct dv2_algo_info *info = prop_chgalgo_get_drvdata(pca);
	struct dv2_algo_data *data = info->data;
...
static SIMPLE_PCA_ALGO_DESC(pca_algo_hvdv2, pca_dv2_ops);
					   ...
					   int prop_cnt)
{
	...
	for (i = 0; i < prop_cnt; i++) {
		if (unlikely(!props[i].name))
			continue;
		of_property_read_s32_array(np, props[i].name,
					   desc + props[i].offset,
					   props[i].sz);
	}
}
...
static const struct dv2_dtprop dv2_dtprops_u32[] = {
	...
	DV2_DT_VALPROP(ita_term),
	DV2_DT_VALPROP(ita_step),
	DV2_DT_VALPROP(ita_ss_init),
	DV2_DT_VALPROP(ita_ss_step),
	DV2_DT_VALPROP(ita_ss_step1),
	DV2_DT_VALPROP(ita_ss_step2),
	DV2_DT_VALPROP(ita_ss_step1_vbat),
	DV2_DT_VALPROP(ita_ss_step2_vbat),
...
static const char * const dv2_dev_node_name[] = {
	"rt_hvdv2",
};
...
static int dv2_parse_dt(struct dv2_algo_info *info)
{
	...
	for (i = 0; i < ARRAY_SIZE(dv2_dev_node_name); i++) {
		np = of_find_node_by_name(NULL, dv2_dev_node_name[i]);
		if (np) {
			PCA_ERR("find node %s\n", dv2_dev_node_name[i]);
			break;
		}
	}
	if (i == ARRAY_SIZE(dv2_dev_node_name)) {
	...
	desc->ibatocp_half = of_property_read_bool(np, "ibatocp_half");
...
static int dv2_algo_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s(%s)\n", __func__, PCA_HVDV2_ALGO_VERSION);
	...
	if (ret < 0) {
		dev_err(info->dev, "%s parse dt fail(%d)\n", __func__, ret);
		return ret;
	}

	info->pca = prop_chgalgo_device_register(info->dev,
						 &pca_algo_hvdv2_desc, info);
	if (IS_ERR_OR_NULL(info->pca)) {
		dev_err(info->dev, "%s reg hv dv2 algo fail(%d)\n", __func__,
			ret);
	...
	data->state = DV2_ALGO_STOP;
	alarm_init(&data->timer, ALARM_REALTIME, dv2_algo_timer_cb);
	data->task = kthread_run(dv2_algo_threadfn, info, "hvdv2_algo_task");
	if (IS_ERR(data->task)) {
		ret = PTR_ERR(data->task);
		dev_err(info->dev, "%s run task fail(%d)\n", __func__, ret);
		goto err;
	}
...
static int __init dv2_init(void)
{
	platform_device_register(&dv2_algo_platdev);
	return platform_driver_register(&dv2_algo_platdrv);
}

static void __exit dv2_exit(void)
{
	platform_driver_unregister(&dv2_algo_platdrv);
	platform_device_unregister(&dv2_algo_platdev);
}
device_initcall_sync(dv2_init);
module_exit(dv2_exit);

MODULE_DESCRIPTION("High Voltage Divide By Two Algorithm For PCA");
MODULE_AUTHOR("ShuFan Lee <shufan_lee@richtek.com>");
MODULE_VERSION(PCA_HVDV2_ALGO_VERSION);
